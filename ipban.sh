...
iptables_rules(){
	...
	if [[ ${IO} == *"O"* ]]; then
		iptables  -A OUTPUT -m geoip -p tcp -m multiport --dports 0:32767 --dst-cc "${GEOIP}" -j "${LIMIT}"
		ip6tables -A OUTPUT -m geoip -p tcp -m multiport --dports 0:32767 --dst-cc "${GEOIP}" -j "${LIMIT}"
		iptables  -A OUTPUT -m geoip -p udp -m multiport --dports 0:32767 --dst-cc "${GEOIP}" -j "${LIMIT}"
		ip6tables -A OUTPUT -m geoip -p udp -m multiport --dports 0:32767 --dst-cc "${GEOIP}" -j "${LIMIT}"		
	fi
